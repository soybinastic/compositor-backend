...
class SceneService:
    ...
    def activate_scene(self, session_id: uuid.UUID, scene_id: uuid.UUID) -> StudioScene:
        session = self._sessions.get_session(session_id)
        self._sessions._assert_not_ended(session)
        scene = self.get_scene(session_id, scene_id)

        if scene.scene_type == SceneType.COUNTDOWN:
            raise CountdownSceneNotActivatableError(
                'Countdown scenes cannot be activated in this phase'
            )
